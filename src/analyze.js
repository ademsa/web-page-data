export default function Analyze(doc) {
  ...
  doc.querySelectorAll("meta").forEach((item) => {
    ...
    if (
      item.attributes.content !== undefined &&
      item.attributes.content.value !== ""
    ) {
      obj.content = item.attributes.content.value;
    }
    if (
      item.attributes.property !== undefined &&
      item.attributes.property.value !== ""
    ) {
      obj.property = item.attributes.property.value;
    }
    if (
      item.attributes.charset !== undefined &&
      item.attributes.charset.value !== ""
    ) {
  ...
  doc.querySelectorAll("table").forEach((table) => {
    ...
    table.querySelectorAll("tbody").forEach((tbody) => {
      ...
      if (
        trs.length > 1 &&
        trs[1].querySelectorAll("th").length > 0 &&
        trs[1].querySelectorAll("td").length > 0
      ) {
        let row = [];
        trs.forEach((tr) => {
        ...
        if (row.length > 0) {
          // Adding check in case tr contains th
